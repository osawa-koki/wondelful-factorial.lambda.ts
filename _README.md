...
```bash
wondelful-factorial.lambda.ts$ sam build
```
...
```bash
wondelful-factorial.lambda.ts$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
wondelful-factorial.lambda.ts$ sam local start-api
wondelful-factorial.lambda.ts$ curl http://localhost:3000/
```
...
```bash
wondelful-factorial.lambda.ts$ sam logs -n HelloWorldFunction --stack-name wondelful-factorial.lambda.ts --tail
```
...
```bash
wondelful-factorial.lambda.ts$ cd hello-world
...
```bash
sam delete --stack-name wondelful-factorial.lambda.ts
```
